class Graphics:
    ...
    def display(env):
        ...
        final_list = [["___" for _ in range(len(env.map[0])+2)]]
        for row in env.map:
            final_list.append([" | "]+list(replace_dict[tile] for tile in row)+[" | "])
        final_list.append(["___" for _ in range(len(env.map[0])+2)])
        final_list[0][0] = final_list[-1][0] = " __"
        final_list[0][-1] = final_list[-1][-1] = "__ "

        i, j = env.agent_loc
        if final_list[i+1][j+1] == " d ":
            final_list[i+1][j+1] = "dX "
        else:
            final_list[i+1][j+1] = " X "
